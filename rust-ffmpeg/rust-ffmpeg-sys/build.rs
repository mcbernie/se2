...
fn check_features(
    ...
    if env::var("TARGET").unwrap() != "arm-unknown-linux-gnueabihf" {
        ...
        let stdout_raw = Command::new(out_dir.join(&executable))
            ...
            .expect("Check failed")
            .stdout;
        stdout = str::from_utf8(stdout_raw.as_slice()).unwrap().to_string();
    } else {
...
fn search_include(include_paths: &Vec<PathBuf>, header: &str) -> String {
    ...
    if let Ok(root) = env::var("SYSROOT") {
        format!("{}/usr/include/arm-linux-gnueabihf/{}",root, header)
    } else {
...
fn main() {
    ...
    if env::var("TARGET").unwrap() == "arm-unknown-linux-gnueabihf" {
        let root = env::var("SYSROOT").unwrap();
        builder = builder.clang_arg("-target")
            .clang_arg("arm-linux-gnueabihf")
            .clang_arg(format!("-I{}/usr/include/arm-linux-gnueabihf",root))
    }
